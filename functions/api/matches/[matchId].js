...
export async function onRequestGet({ request, params, env }) {
  ...
  try {
    ...
    const existingMatch = await getMatch(scopedEnv, matchId);
    const result = await refreshStoredMatch(scopedEnv, matchId, {
...
export async function onRequestPost({ request, params, env }) {
  ...
  try {
    ...
    const result = await refreshStoredMatch(scopedEnv, params.matchId, {
...
export async function onRequestPatch({ request, params, env }) {
  ...
  try {
    ...
    if (Array.isArray(body.manualRoster)) {
      ...
      const match = await updateMatchManualRoster(scopedEnv, params.matchId, body.manualRoster);
      ...
      return json({ match, matches: await getMatches(scopedEnv), auditLogs: await getAuditLogs(scopedEnv), detail: await getMatchDetail(scopedEnv, params.matchId), message: "已保存手动补全阵容" });
    }
    if (body.winnerSide) {
      const before = await getMatch(scopedEnv, params.matchId);
      const match = await updateMatchWinner(scopedEnv, params.matchId, body.winnerSide);
      if (!match) return json({ error: "比赛不存在" }, { status: 404 });
      await logAuditAction(scopedEnv, {
        action: "set_winner",
        matchId: params.matchId,
      ...
      return json({ match, matches: await getMatches(scopedEnv), auditLogs: await getAuditLogs(scopedEnv), message: `已手动指定${body.winnerSide}胜` });
    }
    const status = body.status;
    if (!["待确认", "已确认", "已入库", "已驳回"].includes(status)) return json({ error: "比赛状态无效" }, { status: 400 });
    if (status === "已入库") {
      const currentMatch = await getMatch(scopedEnv, params.matchId);
    ...
    const before = await getMatch(scopedEnv, params.matchId);
    const match = await updateMatchStatus(scopedEnv, params.matchId, status);
    ...
    return json({ match, matches: await getMatches(scopedEnv), auditLogs: await getAuditLogs(scopedEnv) });
  } catch (error) {
